...
async function detectAuthEnforcement(): Promise<boolean> {
  try {
    ...
    console.log(`  ${RED}✗${NC} auth-enforcement probe failed (${message})`);
    return false;
  }
}
...
const CODE_TAG = /<\/?code>/g;

function followsHintOutputContract(hint: string): boolean {
  if (hint.length > MAX_HINT_RESPONSE_CHARS) return false;
  if (/[<>]/.test(hint.replace(CODE_TAG, ''))) return false;

  let unclosed = 0;
  for (const tag of hint.match(CODE_TAG) ?? []) {
    if (tag === '<code>') {
      unclosed += 1;
    } else if (unclosed === 0) {
      return false;
  ...
  if (Array.from(hint).length > MAX_HINT_CODE_POINTS) return true;
