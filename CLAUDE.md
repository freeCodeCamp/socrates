...
The trade is deliberate in both directions. Before this ordering the limiter keyed on `body.userId` *before* auth ran, so an unauthenticated caller could drain a victim's bucket by sending the victim's `userId`. That is now impossible — but API-key guessing and malformed-body floods are no longer metered at the app layer.
